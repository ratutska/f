...
from app.almecen import Pendientes
...
def salir():
    ...
    ctx = PagContexto(usuario= current_user, txt_submit='salir', funcion='salir')
...
def agregarPendientes():
    ...
    ctx = PagContexto(funcion='agregarPendientes', formulario= tareaForm, txt_submit='agregar', usuario= current_user)
...
@login_required
def hecho(id):
    Pendientes.cambiarEstado(current_user, id)
    return redirect(url_for('agregarPendientes'))
